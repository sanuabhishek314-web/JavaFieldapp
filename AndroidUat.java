        ...
        try {
            ...
            try {
                ...
                driver.executeScript("mobile: clickGesture",
                        Map.of("x", size.getWidth() / 2, "y", (int) (size.getHeight() * 0.1)));
                Thread.sleep(500);
            } catch (Exception ignored) {
            }

            // Try multiple methods to ensure keyboard is closed without using BACK
            // navigation
            // using BACK navigation may accidentally go back and dismiss the confirm button
            // popup
            if (Boolean.TRUE.equals(driver.isKeyboardShown())) {
    ...
    private static void selectOtherMeetingDateTab(AndroidDriver driver) {
        ...
        for (int attempt = 1; attempt <= 3; attempt++) {
            for (String xpath : otherMeetingDateXpaths) {
                try {
                    ...
                    for (WebElement tabElement : tabElements) {
                        ...
                        System.out.println(
                                "Attempt " + attempt + ": clicking 'Other Meeting Date' tab using XPath: " + xpath);
            ...
            try {
                ...
                System.out.println("Attempt " + attempt + ": coordinate fallback for 'Other Meeting Date' at (" + x
                        + ", " + y + ")");
                driver.executeScript("mobile: clickGesture", Map.of("x", x, "y", y));
                Thread.sleep(900);
